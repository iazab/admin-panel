  ...
  <tr class="product-card">
    ...
    >
      <div class="flex items-center">
        ...
        <div class="ml-4">
          <div
              class="product-title text-sm font-medium leading-5 text-gray-900"
          >
    ...
    <td
        class="px-6 py-4 border-b border-gray-200 whitespace-nowrap"
    >
      <div class="text-sm leading-5 text-gray-900 flex gap-2">
        {{ product.rating.rate }}
        ...
        <svg xmlns="http://www.w3.org/2000/svg"
             ...
             viewBox="0 0 24 24"
             id="star">
          <path fill="none" stroke="#200E32" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
      ...
      <div class="text-sm leading-5 text-gray-500 flex gap-2">

        {{ product.rating.count }}

        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 1024 1024" id="user">
          <path
    ...
    >
      <div class="flex gap-2">
        ...
        <ModelComponent open-btn-name="Edit"
                        modal-title="Edit Product"
                        action-btn-name="Edit"
                        @model-action="updateProduct"
                        class="action-btn edit-btn"
                        @model-data="openEditProduct(product.id,product)"
        >
        ...
        <ModelComponent open-btn-name="Delete"
                        modal-title="Delete Product"
                        action-btn-name="Delete"
                        @model-action="()=> deleteProduct(product.id) "
                        class="action-btn delete-btn"
        >
          <p>
            Are you sure you want delete product with id #{{ product.id }}
          </p>
        </ModelComponent>
      </div>
...
function openEditProduct(id: number, productValue: ProductData) {
  idOfProduct.value = id;
  ProductItem.value = productValue;
}

async function deleteProduct(id: number) {
  await productApi.delete(id);
  console.log(product);
  console.log("Product Deleted but no action in fake API store")
}

async function updateProduct() {
  await productApi.update(idOfProduct.value, ProductItem);
  console.log(ProductItem);
  ProductItem.value = ""
  console.log("Product edited but no action in fake API store")
}
